...
public class Mathfx
{
	...
	public static float BigBerp(float start, float end, float value)
	{
		value = Mathf.Clamp01(value);
		value = (Mathf.Sin(value * Mathf.PI * (0.2f + 5.5f * value * value * value)) * (1f - value) + value) * (1f + (1.2f * (1f - value)));
		return start + (end - start) * value;
	}
